...
def load_config():
    ...
    try:
        ...
        print(f"  [ERROR] load_config failed: {e}, using defaults")
        return {}
...
def get_market_info():
    ...
    et_now = utc_now.astimezone(ZoneInfo("America/New_York"))  # auto DST
...
def check_buy_trend(rsi, price, sma, ml, sl, hist, avg_vol, cur_vol, macd_threshold=0):
...
async def place_and_confirm(ib, sym, action, quantity, price, mode=None):
    ...
    status_code, _, _, _ = get_market_info()
    is_swing_closed = (mode == "swing" and status_code == "closed")
...
async def run():
    ...
    if not _circuit.available():
        ...
        try:
            ...
            print(f"  [WARN] data.json read failed: {e}, starting fresh")
            dashboard = {}
        dashboard.update({
        ...
        # Read advisor history from log (not from existing data.json which may be stale)
        try:
            _advisor_log = "/root/live_ibkr_dashboard/advisor_log.json"
            _ah = []
            if os.path.exists(_advisor_log):
                for _line in open(_advisor_log):
                    _line = _line.strip()
                    if _line:
    ...
    try:
        ...
        if _circuit.should_notify():
            notify_error("ibkr_etf_gateway", str(e))
            tg(f"⛔ ETF熔断！连续{_circuit.threshold}次连接失败: {str(e)[:100]}")
    ...
    try:
        ...
        print(f"  [WARN] data.json read failed: {e}, metadata reset for this run")
        prev_stats = {"date": now.strftime("%Y-%m-%d"), "trades": 0, "wins": 0, "losses": 0, "pnl": 0.0, "symbols_traded": [], "session_start": now.strftime("%H:%M:%S")}
        trade_history = []
    ...
    if not should_trade:
        ...
        try:
            ...
            print(f"  [WARN] data.json read failed: {e}")
    ...
    try:
        for sym in SYMBOLS:
            ...
            if pos is None:
                ...
                if mode == "oversold":
                    ...
                    checks, all_ok = check_buy_trend(rsi, price, sma, ml, sl, hist, avg_vol, cur_vol, MACD_HIST_THRESHOLD)
                    score = sum(1 for v in checks.values() if v)
        ...
        last_sells_out = {}
        for sym in SYMBOLS:
            if positions.get(sym) is None and sym in prev_last_sells:
